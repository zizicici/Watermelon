...
后台自动备份逐节点读取备份模式的默认数据源（节点覆盖值优先，否则沿用 App 默认值；全部照片和视频、仅照片、仅视频、特定相册），与当前及上一个自然月取交集。设备资源清单在同一次唤醒内按媒体类型分别懒加载、缓存；相册复用备份准备阶段的多相册去重与前后校验，且按月份范围的 cutoff 下推 `creationDate` 谓词，不把整个相册读进内存（选“全部时间”的快捷指令没有 cutoff，仍是完整枚举）。相册失效、权限不足或已保存的选择损坏时该节点失败并记录错误，不扩大到全图库；其他可运行节点继续执行。快捷指令默认使用运行时节点的备份默认数据源，也可显式选择数据源和独立的相册组合；显式参数优先。

自动备份每轮结束后最多发送一条本地汇总通知，按各节点独立的成功、失败通知开关筛选结果。多个节点时列出成功节点和各失败节点的原因，只有一个需要通知的节点时沿用单节点详细文案；无可通知结果时静默。只有完整完成并释放写锁后才记录成功；成功但没有实际传输任何文件的节点只写冷却标记，不计入通知。整轮任务被系统停止或取消时不发送汇总，已完成结果保留在执行日志中。暂停、仓库占用而安全跳过、冷却或网络条件不满足而未开始执行的节点不计入通知。汇总通知使用固定标识 `background-backup.summary`，靠系统「同标识替换已投递通知」的语义让多轮结果不堆积；静默轮次不发送也不清除，上一条结果会一直留到下一条有内容的通知替换它。后台只检查已有通知授权，不弹权限请求；发送通知失败仅写入诊断日志，不改变备份结果。快捷指令沿用自身执行反馈，不发送这些自动备份通知。
