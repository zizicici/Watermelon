...
4. `Watermelon/UI/Backup/BackupRunCommandActor.swift`
...
5. `BackupSessionController` 负责 UI 状态聚合；命令（开始/暂停/停止/恢复）统一进 `BackupRunCommandActor`。
6. `BackupRunCommandActor` 为每次 run 创建独立 `BackupEventStream`，调用 `BackupCoordinator.runBackup(request:eventStream:)`。
7. `BackupCoordinator` 执行权限检查、远端索引同步、按月并发调度、月 manifest flush 与收尾。
...
1. 旧 `BackupExecutor` 已由 `BackupSessionController + BackupRunCommandActor + BackupCoordinator + AssetProcessor` 替代。
2. 代码中不存在 `BackupEngineActor` 类型名（旧文档名）；当前 actor 名称是 `BackupRunCommandActor`。
