...
2. Drop 使用与备份首页相同高度、间距和配色的左右双 Header：左侧标题菜单可切换全部图库或 `Specific Albums`，复用备份的相册多选与预览，选定后显示相册名或相册数量。Drop 独立保留本次会话的范围，切换范围清空已选图片、保留已选文件，多相册中的同一资源只显示一次；投递或文件导入期间禁用范围切换。下方“选项”打开四个独立子菜单，每项再选择“启用 / 关闭”；右侧为“文件”，下方“选择文件”直接打开系统文件选择器。旧的右上角 Filter 与文件按钮不再显示，发送时仍合并图片与文件为同一批项目
