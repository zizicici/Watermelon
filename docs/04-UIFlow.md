...
8. `存储空间` → 紧接投递模式，进入 `StorageUsageViewController`。App 大小独立成组；“文稿与数据”组内先显示合计，再缩进列出缩略图、浏览原件、临时文件、投递暂存、执行日志、本地索引与设置、其他数据。点击明细进入分类详情：浏览器缓存显示占用、容量上限和清理按钮，其余可清理类别另显示可清理大小；本地索引、设置及其他数据只读；“本地索引与设置”详情按文件列出数据库、写入日志、辅助文件、回滚日志、各设置文件及其他本地文件的大小。“其他数据”详情按目录列出远端索引、其他缓存、应用支持数据、其他临时文件、文稿文件和其他资料库数据的大小；展示相对目录便于定位，不提供清理入口。下拉或从详情返回时刷新统计
9. `诊断` → `诊断日志`（跳转 `ExecutionLogHistoryViewController`）；DEBUG 构建额外露出 `Test Crash (Debug)`
