...
1. 左侧：`本地相册`
   ...
   - 照片 / 视频 / 体积汇总
   - 长按 / 菜单：可切换 scope（`iPhone / iPad` → `全部 / 照片 / 视频`，或指定相册）；照片包含 Live Photo，标题在筛选时显示媒体类型
2. 右侧：`节点`
   - 当前连接名称
   - profile 下拉菜单
...
Event Deep Link 填写 `watermelonbackup://shortcuts`。iOS 27 及以上直接进入「设置 → 快捷指令 → 允许快捷指令备份」，较旧系统进入设置首页。链接只导航，不修改开关或执行备份。冷启动与已运行场景共用 `AppDeepLink` 路由；首次安装先完成引导，再进入目标页。已有设置导航栈会复用，重复打开不会叠加同一个选项页。

该 URL Scheme 由包含此改动的版本注册，提交活动时需同时提交该版本。原有 Browser Link 的 Universal Link 和 OneDrive 登录回调保持各自处理流程。
...
2. Drop 使用与备份首页相同高度、间距和配色的左右双 Header：左侧标题菜单可切换 `iPhone / iPad` → `全部 / 照片 / 视频` 或 `Specific Albums`，复用备份的相册多选与预览；标题显示设备与媒体类型，或相册名 / 相册数量，文字过长时使用与备份页一致的滚动文字。Drop 独立保留本次会话的范围，切换范围清空已选图片、保留已选文件，多相册中的同一资源只显示一次；投递或文件导入期间禁用范围切换。下方“选项”打开四个独立子菜单，每项再选择“启用 / 关闭”；右侧为“文件”，下方“选择文件”直接打开系统文件选择器。旧的右上角 Filter 与文件按钮不再显示，发送时仍合并图片与文件为同一批项目
...
`isRemoteSelectionAllowed` 仅在 scope 为 `iPhone / iPad` → `全部` 时允许远端侧选择；指定相册和仅照片 / 仅视频范围只支持本地备份。
...
1. `通用` → `语言` / `相册权限` / `通知权限`；权限行显示当前系统状态（含所选照片、静默通知），首次点击申请授权，之后进入系统设置修改，App 回到前台时刷新状态
2. `节点` → `管理节点`；节点详情可将 `并发数` 设为继承全局默认、显式自动或 `1 / 2 / 3 / 4 / 6 / 8 / 10 / 12 / 16 / 20 / 24`，并提示并发过高可能触发限流或反而降低速度
3. `备份` → `默认数据源`（`全部照片和视频 / 仅照片 / 仅视频 / 特定相册`，默认为全部照片和视频；保存一份相册多选结果，不设命名分组；作为备份和投递模式的默认范围；备份模式可按节点覆盖，首页临时切换不会改写默认值） / `默认并发数` / `允许访问 iCloud 原件`
4. `备份到电脑` → `传输速度`（默认选择 `标准（1 MB/s）`，包括 Pro 用户；`不限速` 需要 Pro；上传与下载共用速度上限，下次连接生效）
5. `自动备份` → `后台自动备份`（Pro） / `自动备份节点`（已启用 / 可用节点计数）；后台任务采用节点备份模式的默认数据源，与最近两个月的时间范围取交集。点击节点进入的是完整的节点详情页，与“管理节点”入口同一个页面，因此同样包含编辑连接、清理残留和删除节点；在此删除或改名会向首页与 More 页广播，不只刷新本列表。“备份模式”提供“默认数据源”，可选“使用 App 默认值”或全部照片和视频、仅照片、仅视频、特定相册。相册可多选，清空切回全部；相册失效或权限不足时，节点列表及节点备份模式的默认数据源行显示红色原因提示，进入数据源页面引导修正。相册校验在后台线程执行，结果回主线程刷新。返回页面或 App 回到前台时重新检查。已启用后台备份的节点，列表摘要包含所选数据源；未启用的只显示后台备份状态。节点详情分别提供“成功通知”和“失败通知”开关，默认开启；没有总通知开关，系统授权由“通用”的通知权限入口管理。每轮结束后最多发送一条汇总通知，按各节点开关筛选成功和失败结果；单节点保留详细文案，无可通知结果则静默。整轮成功但没有实际传输任何文件时不发成功通知。汇总通知使用固定标识，新一轮结果替换上一条而不是堆积。系统停止或取消整轮任务时不发结果通知
6. iOS 27 起显示 `快捷指令` → `允许快捷指令备份`（默认启用，Pro 标记）；Section 外部页脚仅向免费用户显示“剩余 N 次免费试用”，Pro 用户不显示外部页脚。仅限制“进行备份”动作，Pro 不限次数，免费用户成功完成 5 次后停止使用，失败或取消不扣次数；全新安装重置试用，切换开关不重置次数
7. `画中画` → `画中画进度`（Pro）；开启且持有 Pro 时再露出 `声音`
8. `投递模式` → 首项为 `使用说明`，重新展示首次进入时的五步教程，先介绍按需传送照片、视频和文件的用途，以及与备份模式在备份记录和去重上的区别，再展示操作说明；可随时关闭，完成后返回设置页。其后为图片与视频导出选项
9. `存储空间` → 紧接投递模式，进入 `StorageUsageViewController`。App 大小独立成组；“文稿与数据”组内先显示合计，再缩进列出缩略图、浏览原件、临时文件、投递暂存、执行日志、本地索引与设置、其他数据。点击明细进入分类详情：浏览器缓存显示占用、容量上限和清理按钮，其余可清理类别另显示可清理大小；本地索引、设置及其他数据只读；“本地索引与设置”详情按文件列出数据库、写入日志、辅助文件、回滚日志、各设置文件及其他本地文件的大小。“其他数据”详情按目录列出远端索引、其他缓存、应用支持数据、其他临时文件、文稿文件和其他资料库数据的大小；展示相对目录便于定位，不提供清理入口。下拉或从详情返回时刷新统计
10. `诊断` → `诊断日志`（跳转 `ExecutionLogHistoryViewController`）；DEBUG 构建额外露出 `Test Crash (Debug)`

再叠加 MoreKit 自带的 `membership / contact / appjun / about` 段落。

指定相册需要完整照片权限。使用该范围时，任一相册失效都会保留整组选项和缓存名称，停止该范围的操作，并显示具体失效相册及“重新选择相册”入口；不会自动丢弃失效项或退回整个照片库。纯文件投递不依赖照片相册校验。修改已保存且失效的相册选择会同步修复它的来源：当前节点有「特定相册」覆盖时写回该节点的覆盖值，否则写回 App 默认值；两者都不会改动另一方。节点覆盖不是「特定相册」时，其相册列表只是切换媒体类型用的记忆，不参与修复，首页此时只改临时范围。节点写入被执行中、维护中或连接中拒绝时弹错误并保持首页原样，避免显示与落盘分叉。切换到设备数据源仍保留相册选择。

App 相册选择器只展示当前可用的相册，点击“完成”才保存修正后的选择，取消不会改写原组合。选择器支持“清除”，也允许取消最后一个相册后点击“完成”；主动提交空选择会切回“全部照片和视频”。设置页同时清空保存的相册组合，首页和投递模式清空当前范围。因相册失效而变空的列表不能直接提交为清空，仍需用户明确清除或重新选择；快捷指令的空相册参数继续抛错。

iOS 27 快捷指令“进行备份”提供节点、时间范围和展开的“数据源”参数。数据源通过 `LocalDataSourceEntity` 提供“全部照片和视频 / 仅照片 / 仅视频 / 特定相册”，默认选“使用节点默认值”，执行时解析节点的备份模式默认数据源，未覆盖时沿用 App 设置；也可显式选择四种数据源。只有选中“特定相册”时，条件摘要才显示可多选的“相册”参数；`LocalAlbumEntity` 查询照片库，显式选择“特定相册”时，每条快捷指令独立保存所选相册标识，不修改节点或 App 的相册组合。相册名称另行缓存用于失效提示；解析已保存的标识时保留失效项，让执行整次抛错。时间范围默认最近两个日历月，可改为“全部时间”，系统进度标题为“向 XXX 备份”。

存储统计按本地文件逻辑大小估算；文稿与数据总量按设备号、inode 与创建时间去重，共享原件不会重复计入总量，数值可能与系统设置不同。读取失败显示“无法统计”。`AppCacheManager` 在启动最早阶段记录自有临时文件的身份、大小与修改时间；仅清理前次运行遗留且未变化的文件及旧投递 session，不依赖文件日期判断是否活跃。原图浏览数据源和执行日志 writer 通过 `LocalCacheFileAccess` 保留正在使用的文件；长时间扫描和排序不持有全局锁，仅在删除前短暂检查保护状态。手动降低原图缓存上限同样保留正在使用的文件。缩略图清理后重新统计，仍有文件或统计失败则报告清理失败。启动时已确认存在多个链接的旧导入别名允许仅修改时间变化，仍检查身份、大小和使用保护。

完成一次执行后，若距 DB 创建已满 7 天，会通过 `RatingPromptService.requestReviewIfEligible(in:)` 调用 `AppStore.requestReview(in:)` 请求系统评价框。

节点备份默认数据源在连接节点或有效默认值变化时应用，包括在设置里改动 App 默认值——备份模式会立即跟随并清空首页当前选择，不必等下次启动。投递模式始终沿用 App 默认数据源，不提供分节点覆盖，且只在浏览器重新应用时才跟随变化。普通刷新不会覆盖首页的临时选择，切换节点也不会重置投递范围；任务执行期间延后更新范围。后台备份固定读取节点备份默认值，不受首页临时选择影响。
