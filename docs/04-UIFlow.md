...
3. `备份` → `默认数据源`（`全部照片和视频 / 仅照片 / 仅视频 / 特定相册`，默认为全部照片和视频；保存一份相册多选结果，不设命名分组；用于下次启动时备份和投递模式的初始范围，首页临时切换不会改写默认值） / `默认并发数` / `允许访问 iCloud 原件`
...
指定相册需要完整照片权限。使用该范围时，任一相册失效都会保留整组选项和缓存名称，停止该范围的操作，并显示具体失效相册及“重新选择相册”入口；不会自动丢弃失效项或退回整个照片库。纯文件投递不依赖照片相册校验。修改 App 中已保存且失效的相册选择会同步修复 App 的保存结果，切换到设备数据源仍保留相册选择。
...
iOS 27 快捷指令“进行备份”提供节点、时间范围和展开的“数据源”参数。数据源通过 `LocalDataSourceEntity` 提供“全部照片和视频 / 仅照片 / 仅视频 / 特定相册”，初始类型采用 App 设置值。只有选中“特定相册”时，条件摘要才显示可多选的“相册”参数；`LocalAlbumEntity` 查询照片库，每条快捷指令独立保存所选相册标识，不读取或修改 App 的相册组合。相册名称另行缓存用于失效提示；解析已保存的标识时保留失效项，让执行整次抛错。时间范围默认最近两个日历月，可改为“全部时间”，系统进度标题为“向 XXX 备份”。
