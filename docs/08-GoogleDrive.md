...
Foreground and background Google Drive writers may reuse the generic service's bounded recent lease confidence for recoverable writes. Confidence expiry falls back to a remote proof; destructive writes may reuse confidence only for an attended lease, while background destructive writes always prove ownership remotely. Lock cursor scans reuse the complete metadata returned by the folder listing and download only the active record body; fixed-ID recovery paths still fetch metadata by ID.
