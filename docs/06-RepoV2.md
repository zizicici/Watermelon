...
4. **维护与 compaction**：`RepoCompactionService` 独立执行三阶段 compaction（checkpoint → commit GC → snapshot GC），由 `RepoMaintenanceStartupRunner` 在 startup maintenance 时驱动；`RepoCheckpointService` 对 clean outcome 月份写 checkpoint snapshot；commit GC 使用 covered-bound prefix + planner 推荐 + candidate revalidation + post-delete equivalence 删除 commit 前缀；snapshot GC 使用 accepted-covers-candidate covered dominance + keepN + post-delete verification 删除多余 snapshot。
...
- `idx_local_assets_fingerprint_candidates` partial index
- `RepoStateRecord` GRDB record

### 远端布局 + 编排（`Shared/Services/Repo/`）

- `RepoLayout` — 路径常量 + filename 编解码（snapshot/commit）；snapshot/commit 用完整 writerID UUID（GC 安全）；数据文件名需要 writer 后缀时使用 `writerIDShort` / run prefix 约定；liveness 路径 helper 仅用于旧目录名解析和 orphan cleanup
...
- `SnapshotWriter` — jsonl；通过 `MetadataCreateGate` 写最终 snapshot path；`.bestEffortRetry` 可接受并依赖 commit log 重建
- `SnapshotReader` — 流式解析
...
- `RepoCompactionPlanner` / `RepoCompactionService` / `RepoCheckpointService` — 独立三阶段 compaction（checkpoint → commit GC → snapshot GC）；checkpoint 仅对 clean outcome 月份执行
- `RepoRetentionDeletePreflightService` / `RepoRetentionCommitDeleteExecutor` — commit GC：covered-bound prefix + planner 推荐 + candidate revalidation + post-delete equivalence
- `RepoSnapshotDeletePreflightService` / `RepoSnapshotDeleteExecutor` — snapshot GC：accepted-covers-candidate covered dominance + keepN + post-delete verification
- `RepoSnapshotProtectionSet` — snapshot GC 保护集：accepted baseline + N most recent（`snapshotFallbackKeepCount = 2`）
- `BackupV2RuntimeServices` — 一个 run 期间 V2 服务的容器；`shutdown()` 关闭 metadata client
...
- `BackupParallelExecutor` worker 通过 `V2MonthSession.loadOrCreate(...)` 打开月份状态（当前写路径已 V2-only；缺少 `v2Services` 时抛 `requiresForegroundMigration`）
- `V2MonthSession` 内：
  - `V2MonthIndexes` 持有 in-memory resources / assets / links / pending asset+tombstone 集合
  - `flushToRemote` drain append-only commit chunks 并返回 durable deltas；不写 snapshot、不做 checkpoint 或 GC
  - commit 已落盘但 post-commit 失败时通过 `MonthDurableCommitPartial(delta:flushError:)` 携带已落盘 delta；`FlushError.postCommitFailed` 包装实际错误
- Row-writing asset 在 result 返回前写 per-asset commit；`BackupParallelExecutor` 与 `BackgroundBackupRunner` 同步采用「每 200 个非 failed 结果 flush 一次」（`BackupV2Constants.batchFlushInterval = 200`）的 commit-cadence flush；月末仍有兜底 flush
...
- 代表性覆盖：`CoveredRangesTests`、`RepoLayoutTests`、`RepoStateRecordTests`、`CommitOpMapperTests`、`SnapshotRowMapperTests`、`IntegrityCheckTests`、`CommitLogParseTests`、`RepoMaterializerRoundTripTests`、`RepoMaterializerReadRaceTests`、`V2FlushTests`、`BootstrapStateMachineTests`、`ConcurrentBootstrapRaceTests`、`RepoVerifyMonthServiceTests`、`RemoteIndexFormatRouteDecisionTests`、`RepoCheckpointServiceTests`、`RepoCompactionPlannerTests`、`RepoCompactionPolicyTests`、`RetentionInvariantEvaluatorTests`、`RepoSnapshotProtectionSetTests`、`BackendCapabilityFailClosedTests`
...
- `V2MonthCommitFlusher` 写 append-only commit chunks，`V2SessionCommitTracker` 跟踪 session-local covered ranges
...
- BackupV2RuntimeServices 持有专用 `metadataClient`：commit / snapshot 不再与 worker upload 抢占同一 connection；shutdown 时 disconnect
...
- `BackupV2Constants.batchFlushInterval` 公共常量，BackupParallelExecutor / BackgroundBackupRunner 共用（Iter 3 时为 `10`，现已调为 `200`）
...
- BackupParallelExecutor / BackgroundBackupRunner 把 batch flush 触发条件从 `result.status == .success` 放宽到 `result.status != .failed`,让 cached-reuse `.skipped` 路径(monthStore 已 dirty)也参与 batch flush cadence（当时为 10-asset snapshot cadence，现已改为 200-asset commit-only cadence）
...
**Per-asset commit 已落地**：旧 batch commit / optimistic subtraction 机制已删除。当前每个 row-writing asset 写 commit；batch flush 是 commit-only。compaction（checkpoint / commit GC / snapshot GC）由独立的 `RepoCompactionService` 执行，剩余边界见 `docs/05-OpenIssues.md` §12。
...
- 当时的 `MonthManifestStore.writeV2Snapshot` 曾补写 primary + 所有备用 logical name；该 V2 sqlite bridge 已被 Iter 7 删除，后续 snapshot 由 `SnapshotWriter` 在 compaction checkpoint 阶段输出
...
### Iter 10 当前实现：独立 compaction

- `RepoCompactionPolicy.default` 当前阈值：`checkpointCommitThreshold = 5000`、`checkpointByteThreshold = 16 MiB`、`retentionStalenessThresholdSeconds = 24h`、`snapshotFallbackKeepCount = 2`（`BackupV2Constants`）
- `V2MonthSession.flushToRemote` 是 commit-only：drain append-only commit chunks，不写 snapshot、不做 checkpoint 或 GC
- `RepoCompactionService` 独立执行三阶段 compaction：
  1. **Checkpoint**：`RepoCheckpointService` 对 clean outcome 月份按 replay commit 数或 bytes 写 checkpoint snapshot，验证 readback 和 acceptance
  2. **Commit GC**：`RepoRetentionDeletePreflightService` + `RepoRetentionCommitDeleteExecutor` 使用 covered-bound prefix + planner 推荐 + candidate revalidation 删除 commit 前缀，post-delete equivalence 校验
  3. **Snapshot GC**：`RepoSnapshotDeletePreflightService` + `RepoSnapshotDeleteExecutor` 使用 accepted-covers-candidate covered dominance + keepN（含 accepted covered-max）删除多余 snapshot，post-delete verification
- `RepoMaintenanceRuntimeBuilder` 在 enabled 模式下运行 `OrphanMetadataCleanup.sweepOwnStagings()` 清理当前 writer 的 staging 文件，后台跑 `sweep()` 清理 age-based orphan staging
- `RepoMaintenanceStartupRunner` 在 enabled 模式下调用 `RepoCompactionService.compactStartupMonths()` 对推荐月份执行 startup compaction
- 正常 V2 runtime build 会启动 maintenance；`verifyMonth` 为应用 tombstone 临时打开 runtime 时显式禁用 startup maintenance

当前仍未实现 / 延后项：

1. **物理数据文件 GC**：跨 writer 安全的版本（plan §0 / §6.5 明确不做）；commit 前缀 GC 已有保守实现
2. **gcOrphan op + 物理文件 refcount + 「从备份删除 asset」UI**
3. **`reportMissing` / `reportTampered` / `commitCorruption` op**
4. **WebDAV `If-None-Match` probe + 缓存**：当前直传 `*`，服务端不支持时回 412 当作 alreadyExists（错误归类正确，语义略有偏差）
5. **持久化本地 V2 materialized cache**（cold start 进入毫秒级）
6. **月级 materialize 并发深化**
7. **E2EE**（modes: plain / e2ee-content-visible / e2ee-private）
8. **audit / repair 完整 UI**（兜底 pre-1.2.0 客户端误用造成的 desync）
9. **全局 / 跨月 compaction snapshot**（当前只有 per-month checkpoint）
10. **「合并历史 writer」UI**（处理 Keychain 丢失场景）
11. **`verifyMonth` `partiallyMissing` 类提供 re-upload 动作**

## 关键假设 / 限制

1. `min_app_version "2.0.0"` 是 `RepoLayout.minAppVersionPlaceholder`；发布时换实际下一版本号
2. 升级**不可逆**：phase 2 翻转后已升级客户端永远无法回退到 V1
3. V2 已消除 deferred batch commit 窗口；flush 是 commit-only。当前 compaction 保守删除被 accepted checkpoint covered-bound + post-delete verification 共同保护的 commit 前缀和 covered-dominance 保护的 snapshot
4. **多端并发**：V2 基于 append-only per-writer commits + covered-max materialization + clean outcome gates，不保证零冲突（特别是 WebDAV）；plan §12 已声明
5. **pre-1.2.0 V1 客户端**：协议无 sentinel 锁出，依赖用户主动升级所有设备

## Arch-VII A-I — Error Semantics & Grace Read (2026-05)

Three boundary abstractions removed per-call-site re-derivation of remote error meaning.

- **`RemoteFault`** (`Shared/Services/SMB/RemoteFault.swift`) — one classification at the storage boundary: `notFound` / `retryable` / `cancelled` / `terminal`, via `classify(_:)` built on the existing primitives (`RemoteWriteClassifier.isCancellation`, `isStorageNotFoundError`, `isTransientVerifyFailure`). Buckets mirror the only distinctions V2 consumers act on; permission/quota/auth/config all terminate the same.
- **`BackupErrorChain.nsErrorChain(_:)`** — the single NSError collector. The four prior ad-hoc walkers (`RemoteWriteClassifier`, `S3ErrorClassifier`, `SMBErrorClassifier`/AMSMB2, `AssetProcessor+Upload`) delegate to it (ObjectIdentifier dedup; descends every V2 wrapper type and `RemoteStorageClientError.underlying`). S3 not-found lives once in `S3ErrorClassifier.isNotFound` (`NoSuchBucket` is explicitly not absence); `S3Client` and `RemoteStorageErrorClassifier` delegate.
- **`GracefulRead` / `AbsenceKind`** (`Shared/Services/Repo/GracefulRead.swift`) — `read` (self-write read-after-write, absence-as-value) and `retryWithinGrace` (loop primitive with `Backoff.fixed|.exponential` for sites that map their own verdict or rethrow past the deadline). Replaces hand-written `while Date() < deadline` loops in MigrationMarkerStore, RepoCanonicalIdentity, RepoIdentityAuthority, RepoBootstrapInspectionFSM, VersionManifestStore, AssetProcessor+Upload. `AbsenceKind` names whether concluded absence is `genuinelyAbsent` (deadline exhausted / zero-grace) or `visibilityLag`. Each site's terminal decision, backoff schedule, zero-grace short-circuit, and error propagation are preserved exactly — only loop mechanics are unified. Sites whose grace loop computes the deadline without a `grace>0` guard (RepoCheckpointService), the byte-equality verifiers (MetadataWriteVerifier), and bespoke set-completeness loops (IdentityClaimStore election, RepoMaterializer listed-then-404, RestoreService candidate-set) are deliberately left bespoke — see `Loop/Arch-VII/deferred.md`.

Open-error de-layering: `BackupV2RuntimeOpenFailureKind` and `BackupV2RuntimeOpenFailure` removed; the BG lease carries the raw build error and the runner classifies it directly via `runtimeOpenFailureDisposition(_ error:)`, while translation to `BackupCompatibilityError` happens once in `compatibilityError(for:)`. The shared "recoverable transport vs damaged V2" ladder is deduped into `mapUnreadableUnderlying`. `BackupCompatibilityError` (UI-consumed) and `RepoBootstrapInspectionFSM`'s direct throwing are unchanged.

Capability `.unknown` is fail-closed in both directions — byte-exact presence (no name merge) via `usesExactNameMatchingForPresence`, case-folded collision (no allowed collision) via `foldsCaseForCollisionAvoidance` — and every consumer routes through those helpers. Guarded by `BackendCapabilityFailClosedTests`. SFTP name-case and WebDAV liveness-MOVE runtime probing remain deferred.
