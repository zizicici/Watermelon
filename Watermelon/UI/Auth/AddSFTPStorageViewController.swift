...
final class AddSFTPStorageViewController: UIViewController {
        ...
        saveTask = Task { [weak self] in
            ...
            do {
                ...
                await MainActor.run {
                    ...
                    // can claim the shared execution lease while host-key capture / verifyBasePathWritable
                    // are awaiting the network, so the initial gate check is not enough. Gate on the
                    // actual mutation target (`draft.baseProfile?.id`) so an Add flow that adopts an
                    // existing duplicate row is gated identically to an explicit Edit.
    ...
    private func persistProfile(draft: ValidatedDraft, fingerprint: String) throws -> ServerProfileRecord {
        ...
        if let oldRef = draft.baseProfile?.credentialRef, oldRef != draft.credentialRef {
