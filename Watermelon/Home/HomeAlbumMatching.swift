...
enum HomeAlbumMatching {
    ...
    static func buildRemoteItems(from snapshot: RemoteLibrarySnapshot) -> [RemoteAlbumItem] {
        ...
        let resourcesByMonthHash = Dictionary(uniqueKeysWithValues: snapshot.resources.map { resource in
            (
                ResourceLookupKey(
                    year: resource.year,
        ...
        linksByAssetID.reserveCapacity(snapshot.assets.count)
        for link in snapshot.assetResourceLinks {
            linksByAssetID[link.assetID, default: []].append(link)
        }

        var result: [RemoteAlbumItem] = []
        result.reserveCapacity(snapshot.assets.count)

        for asset in snapshot.assets {
            let links = (linksByAssetID[asset.id] ?? []).sorted { lhs, rhs in
            ...
            contentHashes.reserveCapacity(links.count)
            ...
            guard let representative = chooseRepresentativeResource(resources) else { continue }
            result.append(
                RemoteAlbumItem(
                    id: asset.id,
                    ...
                    mediaKind: detectMediaKind(from: resources),
                    pixelWidth: nil,
                    pixelHeight: nil,
