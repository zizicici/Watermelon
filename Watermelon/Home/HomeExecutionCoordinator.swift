...
final class HomeExecutionCoordinator {
        ...
        do {
            ...
            if session.shouldRunUploadPhase,
               settings.iCloudPhotoBackupMode == .enable {
                ...
                if LocalHashIndexPreflightPolicy
                    .requiresSingleUploadWorker(
                        initialResult: initialResult,
                        uploadAssetIDs: uploadScope,
                        iCloudMode:
                            settings.iCloudPhotoBackupMode
                    ) {
                    let uploadNetworkPending =
                        initialResult.unavailableAssetIDs
                            .union(
                                initialResult
            ...
            if session.requiresCompleteLocalIndexBeforeExecution,
               ...
               settings.iCloudPhotoBackupMode == .enable {
                ...
                while !iCloudResult.unavailableAssetIDs.isEmpty, iCloudAttempt < Self.maxICloudPreflightAttempts {
                    ...
                    iCloudResult =
                        LocalHashIndexPreflightPolicy.merging(
                            iCloudResult,
                            recovery: retry
                        )
                    iCloudAttempt += 1
    ...
    nonisolated static func shouldContinueDownloadAfterVerifyFailure(_ error: Error) -> Bool {
        DownloadVerifyFailurePolicy.canUseCachedSnapshot(after: error)
    }
