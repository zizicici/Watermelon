...
2. `HomeScreenStore`
   聚合首页状态，拥有 `sections / rowLookup / selection / connectionState / executionState`，并把内部变化统一映射为 `.data / .fileSizes / .selection / .execution / .connection / .connectionProgress / .structural`。
...
当前没有独立的用户入口；索引构建由首页每次执行前的预检查自动触发（`HomeExecutionCoordinator.prepareLocalIndexIfNeeded()`）。
