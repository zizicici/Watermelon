...
private final class AssetTransferProgressGate:
    @unchecked Sendable
{
    ...
    func shouldEmit(_ fraction: Double, now: Date) -> Bool {
        lock.withLock {
            if fraction >= 1 {
            ...
            let advancedEnough =
                ...
                >= AssetProcessor.transferProgressMinimumStep
            let waitedEnough =
                now.timeIntervalSince(lastEmitAt)
                >= AssetProcessor.transferProgressMinimumInterval
...
extension AssetProcessor {
    ...
    func performUploadWithRetry(
        ...
        let maxRetry = 3
        var lastError: Error?
        let progressGate = AssetTransferProgressGate()
