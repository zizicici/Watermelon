...
#if canImport(UIKit)
import UIKit
typealias OneDriveAuthPresentationViewController = UIViewController
#elseif canImport(AppKit)
import AppKit
typealias OneDriveAuthPresentationViewController = NSViewController
#endif
...
final class OneDriveMSALService: OneDriveAccessTokenProviding, @unchecked Sendable {
    ...
    func signIn(
        from parentViewController:
            OneDriveAuthPresentationViewController
    ) async throws -> OneDriveInteractiveSignInResult {
        let application = try makeApplication()
