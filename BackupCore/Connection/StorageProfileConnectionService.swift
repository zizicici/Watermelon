...
final class StorageProfileConnectionService: @unchecked Sendable {
    typealias ConfirmSFTPHostKey = @MainActor (
    ...
    struct Hooks: Sendable {
        ...
        static let live = Hooks(captureSFTPHostKey: { host, port in
            try await SFTPClient.captureHostKeyFingerprint(
                host: host,
                port: port
            )
        })
    ...
    func prepareForConnection(
        ...
        let actual = try await hooks.captureSFTPHostKey(
            profile.host,
            profile.port
        )
        ...
        guard await confirmSFTPHostKey(decision, actual) else {
