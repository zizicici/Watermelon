...
struct LocalAssetFingerprintEvaluation {
    let freshRecords: [String: LocalAssetFingerprintRecord]
    let validationAssetIDs: Set<String>
}

enum LocalAssetFingerprintFreshness {
    static func evaluate(
        snapshots: some Sequence<LibraryAssetSnapshot>,
        records: [String: LocalAssetFingerprintRecord]
    ) -> LocalAssetFingerprintEvaluation {
        var freshRecords: [String: LocalAssetFingerprintRecord] = [:]
        freshRecords.reserveCapacity(records.count)
        var validationAssetIDs = Set<String>()
        for snapshot in snapshots {
            guard let record = records[snapshot.localIdentifier] else {
                continue
            }
            if let modificationDate = snapshot.modificationDate,
               modificationDate > record.updatedAt {
                validationAssetIDs.insert(snapshot.localIdentifier)
            } else {
                freshRecords[snapshot.localIdentifier] = record
            }
        }
        return LocalAssetFingerprintEvaluation(
            freshRecords: freshRecords,
            validationAssetIDs: validationAssetIDs
        )
    }
}
...
final class HomeLocalIndexEngine: @unchecked Sendable {
    ...
    private var monthCalendar = LibraryMonthKey.currentPreferenceMonthCalendar()
    private(set) var monthGroupingTimeZone = MonthGroupingTimeZonePreference.frozenCurrent()
    ...
    func reload(
        ...
        monthCalendar = LibraryMonthKey.monthCalendar(preference: monthGroupingTimeZone)
