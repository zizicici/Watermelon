...
nonisolated final class PhotoLibraryService: @unchecked Sendable {
    ...
    private final class AvailabilityProbeRequestState: @unchecked Sendable {
        private let lock = NSLock()
    ...
    nonisolated func fetchAssets(localIdentifiers: Set<String>) -> [PHAsset] {
    ...
    func exportResourceToTempFileAndDigest(
        ...
        do {
            hashAndSize = try FileDigestService.sha256AndSize(
                of: url,
                cancellationController: cancellationController
            )
        } catch {
    ...
    nonisolated static func resourceFileSize(_ resource: PHAssetResource) -> Int64 {
...
extension PhotoLibraryService: PhotoLibraryAuthorizationProviding {
    func currentAccessState() -> PhotoLibraryAccessState {
        PhotoLibraryAccessState(authorizationStatus())
    }

    func requestAccess() async -> PhotoLibraryAccessState {
        PhotoLibraryAccessState(await requestAuthorization())
    }
}
