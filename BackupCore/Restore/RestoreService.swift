    ...
    var errorDescription: String? {
        ...
        case let .contentHashMismatch(fileName, expectedHashHex, actualHashHex):
            ...
            return String.localizedStringWithFormat(
                String(
                    localized:
                        "restore.error.contentHashMismatch"
                ),
                fileName,
        ...
        case let .fileSizeMismatch(fileName, expectedSize, actualSize):
            ...
            return String.localizedStringWithFormat(
                String(
                    localized:
                        "restore.error.fileSizeMismatch"
                ),
                fileName,
                expectedSize,
                actualSize
        ...
        case .invalidManifestResource(let fileName):
            ...
            return String.localizedStringWithFormat(
                String(
                    localized:
                        "restore.error.invalidManifestResource"
                ),
                fileName
            )
            #else
...
final class RestoreService: Sendable {
    private final class TransferProgressGate: @unchecked Sendable {
        private let lock = NSLock()
        private var lastEmittedFraction = 0.0

        func shouldEmit(_ fraction: Double) -> Bool {
            lock.withLock {
                if fraction < lastEmittedFraction, fraction < 1 {
                    return false
                }
                lastEmittedFraction = max(
                    lastEmittedFraction,
                    fraction
                )
                return true
    ...
    func restoreItems(
        ...
        do {
            ...
            await clientBox.disconnectCurrent()
            return results
        } catch {
            await clientBox.disconnectCurrent()
            throw error
    ...
    private func downloadWithRecovery(
        ...
        let result: NetworkRecoveryResult<Void> = await NetworkRecovery.run(
            ...
            let activeClient = await clientBox.currentClient()
            do {
                try await activeClient.download(
                    remotePath: remotePath,
                    localURL: localURL,
                ...
                if AssetProcessor.isRecoverableNetworkFault(error, profile: profile) {
                    ...
                    await activeClient.disconnectSafely()
                    ...
                    if let fresh = try? makeRemoteClient(profile, password) {
                        do {
                            ...
                            await clientBox.replaceClient(fresh)
    ...
    private func makeTransferProgressHandler(
        ...
        return { fraction in
            ...
            guard progressGate.shouldEmit(clamped) else { return }
    ...
    private actor RestoreClientBox {
        private var client: any RemoteStorageClientProtocol

        init(_ client: any RemoteStorageClientProtocol) {
            self.client = client
        }

        func currentClient() -> any RemoteStorageClientProtocol {
            client
        }

        func replaceClient(_ client: any RemoteStorageClientProtocol) {
            self.client = client
        }

        func disconnectCurrent() async {
            await client.disconnectSafely()
        }
    }
        ...
        try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<String?, Error>) in
            ...
            PHPhotoLibrary.shared().performChanges {
                ...
                if let error {
                    ...
                    continuation.resume(returning: placeholderID.load())
                } else {
